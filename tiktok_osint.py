...
try:
    ...
    TIKTOK_REGION_MAP = {
        ...
        "FR": ("🇫🇷", "فرنسا"), "DE": ("🇩🇪", "ألمانيا"), "TR": ("🇹🇷", "تركيا"),
        "IR": ("🇮🇷", "إيران"), "PK": ("🇵🇰", "باكستان"), "IN": ("🇮🇳", "الهند"),
        "JP": ("🇯🇵", "اليابان"), "KR": ("🇰🇷", "كوريا الجنوبية"),
        "ID": ("🇮🇩", "إندونيسيا"), "MY": ("🇲🇾", "ماليزيا"),
        "TH": ("🇹🇭", "تايلاند"), "VN": ("🇻🇳", "فيتنام"),
        "PH": ("🇵🇭", "الفلبين"), "RU": ("🇷🇺", "روسيا"),
        "UA": ("🇺🇦", "أوكرانيا"), "BR": ("🇧🇷", "البرازيل"),
        "MX": ("🇲🇽", "المكسيك"), "CA": ("🇨🇦", "كندا"),
        "AU": ("🇦🇺", "أستراليا"), "NG": ("🇳🇬", "نيجيريا"),
        "ZA": ("🇿🇦", "جنوب أفريقيا"),
    }
    LANGUAGE_NAMES_AR = {
        "ar": "العربية", "en": "الإنجليزية", "es": "الإسبانية",
        "fr": "الفرنسية", "de": "الألمانية", "it": "الإيطالية",
        ...
        "vi": "الفيتنامية", "tl": "الفلبينية", "un": "غير محدد",
    }
...
COUNTRY_CENTERS = {
    "SA": [24.7136, 46.6753], "AE": [25.2048, 55.2708],
    "EG": [30.0444, 31.2357], "KW": [29.3759, 47.9774],
    "QA": [25.2854, 51.5310], "BH": [26.2235, 50.5876],
    "OM": [23.5880, 58.3829], "JO": [31.9539, 35.9106],
    "LB": [33.8938, 35.5018], "SY": [33.5138, 36.2765],
    "IQ": [33.3152, 44.3661], "YE": [15.3694, 44.1910],
    "PS": [31.9522, 35.2332], "MA": [33.5731, -7.5898],
    "DZ": [36.7538, 3.0588], "TN": [36.8065, 10.1815],
    "LY": [32.8872, 13.1913], "SD": [15.5007, 32.5599],
    "SO": [2.0469, 45.3182], "MR": [18.0790, -15.9650],
    "US": [38.9072, -77.0369], "GB": [51.5074, -0.1278],
    "FR": [48.8566, 2.3522], "DE": [52.5200, 13.4050],
    "IT": [41.9028, 12.4964], "ES": [40.4168, -3.7038],
    "NL": [52.3676, 4.9041], "BE": [50.8503, 4.3517],
    "CH": [46.9480, 7.4474], "SE": [59.3293, 18.0686],
    "NO": [59.9139, 10.7522], "DK": [55.6761, 12.5683],
    "FI": [60.1699, 24.9384], "PT": [38.7223, -9.1393],
    "AT": [48.2082, 16.3738], "PL": [52.2297, 21.0122],
    "GR": [37.9838, 23.7275], "TR": [41.0082, 28.9784],
    "IR": [35.6892, 51.3890], "PK": [24.8607, 67.0011],
    "IN": [28.6139, 77.2090], "BD": [23.8103, 90.4125],
    "AF": [34.5553, 69.2075], "CN": [39.9042, 116.4074],
    "JP": [35.6762, 139.6503], "KR": [37.5665, 126.9780],
    "ID": [-6.2088, 106.8456], "MY": [3.1390, 101.6869],
    "SG": [1.3521, 103.8198], "TH": [13.7563, 100.5018],
    "VN": [21.0278, 105.8342], "PH": [14.5995, 120.9842],
    "RU": [55.7558, 37.6173], "UA": [50.4501, 30.5234],
    "BR": [-23.5505, -46.6333], "AR": [-34.6037, -58.3816],
    "MX": [19.4326, -99.1332], "CA": [45.4215, -75.6972],
    "AU": [-33.8688, 151.2093], "NZ": [-36.8485, 174.7633],
    "NG": [9.0765, 7.3986], "ZA": [-26.2041, 28.0473],
    "KE": [-1.2921, 36.8219], "ET": [8.9806, 38.7578],
    "IL": [31.7683, 35.2137],
}
...
FAMOUS_LOCATIONS_TT = [
    "الرياض", "جدة", "دبي", "أبوظبي", "القاهرة", "الإسكندرية",
    "الكويت", "الدوحة", "المنامة", "مسقط", "عمّان", "بيروت",
    "بغداد", "إسطنبول", "الدار البيضاء", "الرباط", "مراكش",
    "الجزائر", "تونس", "طرابلس", "لندن", "باريس", "برلين",
    "نيويورك", "لوس أنجلوس", "تورونتو", "طوكيو", "سيول",
    "كوالالمبور", "جاكرتا",
]
        ...
        if response.status_code == 200 and response.json():
            ...
            result = {
                ...
                "osm_url": (
                    ...
                    f"?mlat={item['lat']}&mlon={item['lon']}&zoom=14"
                ),
            }
...
def build_tiktok_search_links(
    ...
    return {
        ...
        "regional_search": (
            f"https://www.tiktok.com/search?q={quote(regional_terms or query or place)}"
        ),
        "maps": (
            build_map_verification_links(lat, lon)
...
def build_tiktok_user_search(
    ...
    search_query = " ".join(
        x for x in [f"@{username}" if username else "", place, keywords] if x
    ).strip()
    google_query = " ".join(
        x for x in [
            f"site:tiktok.com/@{username}" if username else "site:tiktok.com",
            place,
            keywords,
        ]
        if x
    ...
    return {
        ...
        "tiktok_search": (
            f"https://www.tiktok.com/search?q={quote(search_query)}"
            if search_query
            else "https://www.tiktok.com/search"
        ),
...
# ─────────────────────────────────────────────
# الدالة الرئيسية: استنتاج الموقع من الفيديوهات
# ─────────────────────────────────────────────

def infer_user_location_from_videos(
    ...
    successful_rows = [
        ...
        or r.get("location_created")
        or r.get("author_region")
    ]
    ...
    if probable_code:
        if videos_with_location > 0:
            ...
            # ✅ مُصحَّح: قوس إغلاق إضافي في النهاية
    ...
    if probable_code:
        if location_counter.get(probable_code):
            evidence.append(
                f"أكثر locationCreated تكراراً هو {probable_flag} "
                f"{probable_country_name_ar} بعدد {location_counter[probable_code]} فيديو"
            )
    ...
    if location_counter and len(location_counter) > 1:
        ...
        evidence.append(
            f"الترتيب الأعلى للمواقع: "
            + ", ".join(f"{code} ({count})" for code, count in top_two)
        )
    if language_counter:
        lang, lang_count = language_counter.most_common(1)[0]
        evidence.append(
            f"أكثر لغة محتوى متكررة: "
            f"{LANGUAGE_NAMES_AR.get(lang, lang)} ({lang_count} فيديو)"
        )
    if total_videos and videos_with_location == 0 and videos_with_author_region == 0:
        evidence.append(
            "لا توجد إشارات locationCreated أو author.region كافية داخل الفيديوهات الحالية"
        )
...
# ─────────────────────────────────────────────
# تحليل المنطقة الزمنية من أوقات النشر
# ─────────────────────────────────────────────

def analyze_timezone_from_videos(
    video_results: List[Dict[str, Any]]
) -> Dict[str, Any]:
    ...
    for offset in range(-12, 15):
        ...
        evening = sum(1 for h in local_hours if 18 <= h <= 23)
    ...
    for offset, score in sorted(
        score_by_offset.items(), key=lambda x: x[1], reverse=True
    )[:5]:
        top_offsets.append({
            "offset":            offset,
