...
مدخلات الخوارزميّة
regions_seq: قائمة الدول من آخر 5 فيديوهات
times_seq: قائمة timestamps (UTC) لنفس الفيديوهات
الخطوات
حساب التكرار: الدولة الأكثر تكراراً
تحليل Timezone: من ساعات النشر، نستنتج timezone تقريبي
مطابقة Timezone مع الدولة: إن طابقت، ثقة +15%
التحقّق من التسلسل: إن كانت الدولة الأخيرة مختلفة، ثقة -10%
النتيجة: residence_type (ثابت / مسافر / متنقّل) + confidence (0-100%)
عتبات الثقة
...
Monolith: كل الكود في `app.py` (2,740 سطر) — يحتاج تقسيم إلى modules
CSS Inline: أكثر من 200 inline style — يحتاج نقل إلى classes
لا اختبارات أوتوماتيكيّة: مخاطرة Regression
لا Logging مركزي: صعوبة التشخيص الإنتاجي
Cache بدائي: `@st.cache_data` فقط — لا Redis
اعتماد على PROXY_CHAIN خارجي: مخاطرة انقطاع الخدمات المجانيّة
---
